...
2. [Analyse de données RT-qPCR avec R-Studio ](#Partie2)
    1. [Charger les packages](#Partie2.1)
    2. [Charger les données](#Partie2.2)
    3. [Permutation de table](#Partie2.3)
    4. [Ajout de metadata ](#Partie2.4)
    5. [Calcul des delta Ct](#Partie2.5)
    6. [Calcul des delta delta Ct](#Partie2.6)
    7. [Plot des résultats](#Partie2.7)
    8. [Annotation des groupes pour chaque echantillon ](#Partie2.8)
3. [RNA-seq](#Partie3) 
...
# Analyse de données RT-qPCR avec R-Studio <a name="Partie2"></a>
## 1. Charger les packages <a name="Partie2.2"></a>
```{r}
...
## 2. Charger les données <a name="Partie2.2"></a>
```{r}
...
## 3. Permutation de table <a name="Partie2.3"></a>
...
```{r}
#data
RNA_dat <- as.matrix(read.csv2("C:/Users/piosteil/Desktop/4-All_git/Cours/TP_Transcripto_N3S6/RNA-seq/RNAseq_countTable.csv", sep = ",", header = TRUE, row.names= 1))
head(RNA_dat)

#metadata 
sampleCondition <- read.csv2("C:/Users/piosteil/Desktop/4-All_git/Cours/TP_Transcripto_N3S6/RNA-seq/SRAs/GSE30352_All_SRAs.txt", sep = "\t")

my_sample <- "Kidney"
ref_sample <- "Cerebellum" #un echantillon aléatoire qui n'est pas le votre
#other_samples <- sampleCondition  |> select(source_name) |> filter(!(source_name %in% c(my_sample, ref_sample)))
#other_samples <- unique(c(other_sample$source_name))
```
...
## Nettoyage echantillon avec peu de count
```{r}
smallestGroupSize <- 3
...
## Genes differentiellement exprimes
...
res <- results(dds, contrast = c('source_name', my_sample, ref_sample), 
                lfcThreshold = 1.5, alpha = 0.05)
res

resOrdered <- res[order(res$log2FoldChange),]
write.csv2(resOrdered, "C:/Users/piosteil/Desktop/4-All_git/Cours/TP_Transcripto_N3S6/RNA-seq/DESeq_Kidney-Cerebellum.csv")
```
