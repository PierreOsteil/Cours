...
```{r}
RTqPCR_dat <- read.csv("RT-qPCR/RT-qPCR_data.txt", header = TRUE, sep = "\t", dec = ",") # argument sep = "\t" pour tabulated, et dec = "," pour décimal 
```
...
```{r}
metadata2 <- read.csv("RT-qPCR/fonction_gène.txt", header = TRUE, sep = "\t")
```
...
RTqPCR_median_GDM_clean <- RTqPCR_median_GDM |> select(echantillon, condition, MeanCt)
#on fucionne les deux tables
RTqPCR_median_dCt <- merge(RTqPCR_median_clean, RTqPCR_median_GDM_clean,
                            by = c("echantillon","condition"))

head(RTqPCR_median_dCt)

#on peut maintenant soustraire les le MedCt de nos replicats techniques pour chaques gène à la moyenne de la valeur du GDM pour cet échantillon

RTqPCR_median_dCt$dCt <- RTqPCR_median_dCt |> mutate(2^-(MedCt-MeanCt))
