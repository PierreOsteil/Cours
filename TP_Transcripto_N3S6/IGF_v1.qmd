...
Vous pouvez également consulter cet ouvrage: [R for Data Science](https://r4ds.had.co.nz/)
...
## 7. Ploting des résultats  <a name="Partie1.7"></a>
...
```{r}
my_sample <- "Ech3" 
RTqPCR_ddCt <- RTqPCR_ddCt |> filter(echantillon %in% c("CTR", my_sample)) |> group_by(Gene) |> mutate(pval = list(t.test(dCt[echantillon == my_sample], dCt[echantillon == "CTR"])$p.value))

head(RTqPCR_ddCt)$pval
```


### 8.2 Plot des genes significatifs
```{r}
RTqPCR_ddCt_signif <- RTqPCR_ddCt |> filter(pval <= 0.05) #moins de genes doivent etre selectionnes
